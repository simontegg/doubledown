...
function DoubleDownIterator (db, options) {
...
util.inherits(DoubleDownIterator, AbstractIterator)

DoubleDownIterator.prototype._next = function (cb) {
  var onEnd = () => {
...
DoubleDownIterator.prototype.createReadStream = function (opts) {
  var returnCount = 0
  ...
  var isFinished = () => (opts.limit && returnCount > opts.limit)
  ...
  var stream = through2.obj(function (data, enc, cb) {
    ...
    if (!isFinished()) {
      ...
      // write to cache
      if (opts.cache) {
        return cache.put(output.key, output.value, err => {
          if (err) {
            return cb(err)
          }
  ...
  var onData = (err, data) => {
    ...
    data.Items.forEach(item => {
      ...
      if (opts.lt && !(item.rkey.S < opts.lt)) {
        filtered = true
      }
...
DoubleDownIterator.prototype.getRange = function(opts, cb) {
  if (opts.gte) {
    if (opts.reverse) {
      opts.end = opts.gte
    } else {
...
module.exports = DoubleDownIterator
